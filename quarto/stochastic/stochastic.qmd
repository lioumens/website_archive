...
```{r}
wheredafuck <- function(foo = "aaa/bbb") {
  new_dir <- fs::dir_create(foo)
  new_jpeg <- fs::path(new_dir, "jpegherenow", ext = "jpg")
  jpeg(new_jpeg)
  plot(1:10)
  dev.off()
  system("pwd")
...
wheredafuck()
fs::path_dir("gray_scott.mp4")
video_file <- fs::path_wd(video_file)
```
...
```{r}
plot(raster(I), axes = F, bty = "n", frame.plot = F,
     interpolate = T)
```
